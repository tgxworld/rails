...
class UrlRewriterTests < ActiveSupport::TestCase
  ...
  def test_port
    assert_equal('http://www.example.com:1271/c/a/i',
      @rewriter.rewrite(@routes, :controller => 'c', :action => 'a', :id => 'i', :port => 1271)
    )
  end

  def test_protocol_with_and_without_separator
    assert_equal('https://www.example.com/c/a/i',
  ...
  def test_user_name_and_password
    assert_equal(
      'http://david:secret@www.example.com/c/a/i',
  ...
  def test_user_name_and_password_with_escape_codes
    assert_equal(
      'http://openid.aol.com%2Fnextangler:one+two%3F@www.example.com/c/a/i',
  ...
  def test_anchor
    assert_equal(
      'http://www.example.com/c/a/i#anchor',
  ...
  def test_anchor_should_be_uri_escaped
    assert_equal(
      'http://www.example.com/c/a/i#anc/hor',
